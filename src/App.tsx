...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const mappedData = data.map(todo => ({ id: todo.id, title: todo.title }));
  ...
  return (
    <div>
      ...
      <h3 style={{border: "1px solid black", width:"20%", padding:"2px"}}>Completed Items count: {completedItems}</h3>
      <h3 style={{border: "1px solid black", width:"20%", padding:"2px"}}>To do Items count: {toBeCompletedItems}</h3>
